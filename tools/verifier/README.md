...
Runner selection attempts at most eight canonical lookups per track and sixteen per invocation, including missing records. Selection conflicts defer only that track. Publication retries conflicts three times with fresh reads, then counts that result as deferred and continues; non-conflict failures still fail the invocation. Publication adds at most 48 further canonical reads (sixteen results times three attempts). Deferred work stays queued for a later invocation.
