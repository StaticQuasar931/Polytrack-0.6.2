...
.map((track)=>[track.id, Object.freeze(track)]));
  ...
  const PROFILE_COSMETIC_OPTIONS={
    ...
    badge:[['auto','Automatic',0],['none','Hidden',0],['betaTester','Beta Tester','beta']]
  };
  ...
  function sanitizeProfileCosmetics(value){
    ...
    return {version:2,theme:pick('theme','classic'),stage:pick('stage','garage'),stripe:pick('stripe','standard'),badge:legacy?'auto':pick('badge','auto'),overridePodium:source.overridePodium===true};
  }
  ...
  function racerCosmeticClasses(entry){
    ...
    return [value.theme!=='classic'?`cosmetic-theme-${value.theme}`:'',value.stage!=='garage'?`cosmetic-stage-${value.stage}`:'',value.stripe!=='standard'?`cosmetic-stripe-${value.stripe}`:'',value.overridePodium?'cosmetic-podium-override':''].filter(Boolean).join(' ');
  }
  function applyProfileCosmetics(element,value){
    ...
    if(safe.theme!=='classic')element.classList.add(`cosmetic-theme-${safe.theme}`);
    if(safe.stage!=='garage')element.classList.add(`cosmetic-stage-${safe.stage}`);
    if(safe.stripe!=='standard')element.classList.add(`cosmetic-stripe-${safe.stripe}`);
  ...
  function profileCustomizerMarkup(entry){
    ...
    const group=(kind,label,description)=>`<fieldset class="profile-cosmetic-group kind-${kind}"><legend>${label}</legend><p>${description}</p><div>${PROFILE_COSMETIC_OPTIONS[kind].map(([id,name,requirement])=>{const unlocked=cosmeticUnlocked(requirement,entry);const note=requirement==='podium'?'Top 3':requirement==='beta'?'Beta Tester':requirement?`${requirement} tracks`:'Available';return `<button type="button" class="profile-cosmetic-choice cosmetic-${kind}-${id} ${current[kind]===id?'selected':''}" data-cosmetic-choice data-cosmetic-kind="${kind}" data-cosmetic-value="${id}" aria-pressed="${current[kind]===id}" ${unlocked?'':`disabled title="Unlock with ${note}"`}><span class="profile-cosmetic-preview" aria-hidden="true"><i></i></span><b>${name}</b><small>${unlocked?note:`Locked · ${note}`}</small></button>`;}).join('')}</div></fieldset>`;
    const livePreview='<section class="profile-row-showcase" aria-label="Live leaderboard row preview"><header><b>LIVE ROW PREVIEW</b><small>First place, another racer, and your own row</small></header><div class="showcase-row showcase-first"><strong>#1</strong><span><b>YOUR RACER</b><small>First place</small></span><em>TOP</em></div><div class="showcase-row showcase-other"><strong>#8</strong><span><b>YOUR RACER</b><small>Seen by other players</small></span><em>RACER</em></div><div class="showcase-row showcase-self"><strong>#8</strong><span><b>YOUR RACER</b><small>Your highlighted row</small></span><em>YOU</em></div></section>';
    ...
    return `<details class="profile-customizer"><summary><span><b>DESIGN YOUR RACER PROFILE</b><small>Build a public look from colors, backdrops, and row patterns</small></span><em>Customize</em></summary><div class="profile-customizer-body"><div class="profile-customizer-intro"><b>YOUR LOOK</b><span>Changes preview instantly. Save once to publish the design to every device.</span></div>${livePreview}${group('theme','COLOR THEME','Choose the color pair used by your profile and Ranked row.')}${group('stage','CAR BACKDROP','Choose the scene behind your car.')}${group('stripe','ROW PATTERN','Choose the full-width design layered over your colors.')}${entry.badges?.betaTester===true?group('badge','DISPLAYED BADGE','Choose the badge shown beside your name.'):''}${podiumChoice}<div class="profile-customizer-actions"><span role="status" aria-live="polite"></span><button class="button" type="button" data-save-profile-cosmetics data-account-id="${escapeHtml(accountId)}">Save for everyone</button></div></div></details>`;
  }
  ...
  function ensureStyles(){
    ...
    style.textContent = `
      ...
      .leaderboard-ui .verified-state{display:inline-flex!important;align-items:center;gap:6px;min-height:24px;padding:3px 7px!important;border:2px solid currentColor!important;border-radius:2px!important}.leaderboard-ui .verified-state .sq-integrity-label{font-size:11px!important;font-weight:700;letter-spacing:.7px}.leaderboard-ui .verified-state.verified{color:#72f0a4!important;background:rgba(19,91,56,.28)!important}.leaderboard-ui .verified-state.pending{color:#ffd166!important;background:repeating-linear-gradient(135deg,rgba(126,82,10,.34) 0 6px,rgba(75,51,12,.2) 6px 12px)!important;border-style:dashed!important}.leaderboard-ui .verified-state img{width:18px!important;height:18px!important}.leaderboard-ui .verified-state.pending img{width:20px!important;height:20px!important}.leaderboard-ui .verified-state.pending .sq-integrity-label::before{content:'WAIT · '}
  ...
  function openRankedProfile(userId){
    ...
    const selectedProfileBadge=cosmeticsForEntry(entry).badge;
    const betaBadge=entry.badges?.betaTester===true&&(selectedProfileBadge==='auto'||selectedProfileBadge==='betaTester')?'<span class="profile-beta-badge" title="Earned by racing during the pre-release Ranked beta">BETA TESTER</span>':'';
  ...
  async function saveRankedProfileCosmetics(accountId,status,button){
    ...
    try{
      ...
      const entry=overallEntriesCache.find((row)=>cleanUserId(row.userId||row.accountId||'')===safeId);
      if(entry)entry.profileCosmetics=cosmetics;
      ...
      if(endpoint&&rankedEdgeAvailable()){
        try{
          ...
          const payload=await response.json().catch(()=>({}));
          if(!response.ok||payload.accepted!==true)throw new Error(payload.error==='cosmetic_locked'?'That style is not unlocked yet.':String(payload.error||'Public update queued.'));
          clearRankedEdgeFailure();
          if(status)status.textContent='Saved for everyone.';
        }catch(error){
          markRankedEdgeUnavailable(String(error&&(error.message||error)));
      ...
      if(button){button.disabled=false;setTimeout(()=>{if(button.isConnected&&button.textContent==='Saved')button.textContent='Save look';},1400);}
    }
  }
  ...
  function decorateNativeLeaderboardCosmetics(){
    ...
    for(const button of buttons){
      ...
      if(!cosmeticClasses.length)continue;
