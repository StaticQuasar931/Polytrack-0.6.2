...
.map((track)=>[track.id, Object.freeze(track)]));
  ...
  function racerCosmeticClasses(entry){
    ...
    return `cosmetic-theme-${value.theme} cosmetic-stage-${value.stage} cosmetic-stripe-${value.stripe}${value.overridePodium?' cosmetic-podium-override':''}`;
  }
  function applyProfileCosmetics(element,value){
    ...
    element.classList.add(`cosmetic-theme-${safe.theme}`,`cosmetic-stage-${safe.stage}`,`cosmetic-stripe-${safe.stripe}`);
  ...
  async function mirrorRaceResult(url, body){
    ...
    try {
      ...
      scheduleLocalPbCloudReconcile(accountId,3000);
  ...
  function decorateNativeLeaderboardCosmetics(){
    ...
    for(const button of buttons){
      ...
      if(!racer)continue;
      button.classList.add('sq-racer-cosmetic',...racerCosmeticClasses(racer).split(' '));
