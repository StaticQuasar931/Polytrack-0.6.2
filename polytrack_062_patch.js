...
.map((track)=>[track.id, Object.freeze(track)]));
  ...
  async function mockPayload(urlObj, method, body){
    ...
    if (urlObj.pathname === '/leaderboardUserEntry' || urlObj.pathname === '/v6/leaderboardUserEntry') {
      ...
      const verifiedEntries=loadedEntries.filter((entry)=>entry.runVerified===true);
      const pendingMine=loadedEntries.find((entry)=>String(entry.accountId||entry.userId||'')===String(accountId||'')&&entry.runVerified!==true);
      const visibleEntries=onlyVerified&&verifiedEntries.length?[...verifiedEntries,...(pendingMine?[pendingMine]:[])]:loadedEntries;
  ...
  function syncIntegrityStateLabels(){
    for(const state of document.querySelectorAll('.leaderboard-ui .verified-state')){
      ...
      label.textContent=status==='verified'?'':status==='replay'?'REPLAY':'UNVERIFIED';
      state.title=status==='verified'?'Run verified':status==='replay'?'Replay checksum checked. Race physics have not been verified.':'Run not verified. You can still race and use Community Ranked.';
      state.setAttribute('aria-label',(date?date+'. ':'')+state.title);
      const icon=state.querySelector('img');
      if(icon){icon.src=status==='verified'?'images/state_verified.svg':'images/state_pending.svg';icon.hidden=status==='replay';}
    }
  }
