...
.map((track)=>[track.id, Object.freeze(track)]));
  ...
  const PROFILE_COSMETIC_OPTIONS={
    ...
    emblem:[['none','None',0],['bolt','Bolt',0],['star','Star',0],['flag','Finish flag',3],['flame','Flame',8],['crown','Crown','podium']],
    title:[['auto','Automatic',0],['none','Hidden',0],['contender','Contender',3],['pbHunter','PB Hunter',3],['trackGrinder','Track Grinder',8],['podiumRegular','Podium Regular','podium'],['betaRacer','Beta Racer','beta']],
    badge:[['auto','Automatic',0],['member','Racing since',0],['none','Hidden',0],['betaTester','Beta Tester','beta']]
  };
  ...
  function sanitizeProfileCosmetics(value){
    ...
    return {version:3,theme:pick('theme','classic'),accent:pick('accent','cyan'),finish:pick('finish','gradient'),stage:pick('stage','garage'),stageTint:pick('stageTint','natural'),stripe:pick('stripe','standard'),emblem:pick('emblem','none'),title:pick('title','auto'),badge:legacy?'auto':pick('badge','auto'),favoriteTrackId,overridePodium:source.overridePodium===true};
  }
  function cosmeticsForEntry(entry){
    const isSelf=cleanUserId(entry?.userId||entry?.accountId||'')===activeRankedAccountId();
    const local=isSelf?readJsonStorage(PROFILE_COSMETICS_KEY,null):null;
    return sanitizeProfileCosmetics(local||entry?.profileCosmetics);
  }
  ...
  function racerCosmeticClasses(entry){
    ...
    return [`cosmetic-theme-${value.theme}`,`cosmetic-accent-${value.accent}`,`cosmetic-finish-${value.finish}`,`cosmetic-stage-${value.stage}`,`cosmetic-stage-tint-${value.stageTint}`,`cosmetic-stripe-${value.stripe}`,`cosmetic-emblem-${value.emblem}`,value.overridePodium?'cosmetic-podium-override':''].filter(Boolean).join(' ');
  }
  function applyProfileCosmetics(element,value){
    ...
    for(const kind of ['theme','accent','finish','stage','stageTint','stripe','emblem'])element.classList.add(`cosmetic-${kind.replace(/[A-Z]/g,(letter)=>`-${letter.toLowerCase()}`)}-${safe[kind]}`);
  ...
  function profileBadgeMarkup(entry,compact=false,cosmetics=null){
    ...
    const pieces=[];
    if(entry?.badges?.betaTester===true&&(selected==='auto'||selected==='betaTester'))pieces.push(`<span class="profile-beta-badge ${compact?'compact':''}" title="Earned during the pre-release Ranked beta">${compact?'BETA':'BETA TESTER'}</span>`);
    const member=memberBadgeLabel(entry);
    if(member&&(selected==='auto'||selected==='member'))pieces.push(`<span class="profile-member-badge ${compact?'compact':''}" title="Racing since ${escapeHtml(new Date(Number(entry.accountCreatedAt)).toLocaleDateString())}">${escapeHtml(member)}</span>`);
    return pieces.join('');
  }
  function profileCustomizerMarkup(entry){
    const current=cosmeticsForEntry(entry);
    ...
    const group=(kind,label,description)=>`<fieldset class="profile-cosmetic-group kind-${kind}"><legend>${label}</legend><p>${description}</p><div>${PROFILE_COSMETIC_OPTIONS[kind].map(([id,name,requirement])=>{const unlocked=cosmeticUnlocked(requirement,entry);const note=requirement==='podium'?'Reach the top 3':requirement==='beta'?'Beta Tester':requirement?`Finish ${requirement} tracks`:'Available';return `<button type="button" class="profile-cosmetic-choice cosmetic-${kind.replace(/[A-Z]/g,(letter)=>`-${letter.toLowerCase()}`)}-${id} ${current[kind]===id?'selected':''}" data-cosmetic-choice data-cosmetic-kind="${kind}" data-cosmetic-value="${id}" aria-label="${escapeHtml(name)}. ${unlocked?note:`Locked. ${note}`}" aria-pressed="${current[kind]===id}" ${unlocked?'':`disabled title="${note}"`}>${['title','badge'].includes(kind)?'':`<span class="profile-cosmetic-preview" aria-hidden="true"><i></i></span>`}<b>${name}</b><small>${unlocked?note:`Locked · ${note}`}</small></button>`;}).join('')}</div></fieldset>`;
    const favoriteOptions=['<option value="">No favorite selected</option>',...Array.from(TRACK_CATALOG.values()).map((track)=>`<option value="${escapeHtml(track.id)}" ${current.favoriteTrackId===track.id?'selected':''}>${escapeHtml(track.name)}</option>`)].join('');
    ...
    const livePreview=`<section class="profile-customizer-preview" aria-label="Live public profile preview"><header><b>LIVE PREVIEW</b><small>This is the design other players receive.</small></header><div class="profile-preview-stage">${carModelPreview(entry.carStyle,entry.carColorId||entry.carColors,accountId)}<span><b>${escapeHtml(safeDisplayName(entry.name||'Racer',accountId))}</b><small class="profile-preview-title" data-cosmetic-title-preview>${escapeHtml(title)}</small></span></div><div class="profile-row-showcase">${showcaseRow('showcase-first',1,'First place','TOP')}${showcaseRow('showcase-other',8,'Seen by others','RACER')}${showcaseRow('showcase-self',8,'Your view','YOU')}</div></section>`;
    const podiumChoice=`<div class="profile-cosmetic-podium"><div><b>TOP THREE COLORS</b><small>Choose whether your design replaces the normal gold, silver, or bronze row.</small></div><button class="button ${current.overridePodium?'selected':''}" type="button" data-cosmetic-podium aria-pressed="${current.overridePodium}">${current.overridePodium?'Use my design':'Keep podium colors'}</button></div>`;
    return `<details class="profile-customizer"><summary><span><b>DESIGN YOUR RACER PROFILE</b><small>Build your public nameplate, car stage, title, and badge</small></span><em>Open studio</em></summary><div class="profile-customizer-body"><div class="profile-customizer-intro"><b>PROFILE STUDIO</b><span>Preview every layer together, then publish it for everyone.</span></div>${livePreview}<div class="profile-customizer-controls">${group('theme','BASE COLORS','Choose the main color pair.')}${group('accent','ACCENT COLOR','Used for borders, marks, and highlights.')}${group('finish','NAMEPLATE FINISH','Choose how the base colors are blended.')}${group('stripe','NAMEPLATE PATTERN','Layer a racing pattern across the full row.')}${group('emblem','FADED EMBLEM','Add one fixed, safe mark behind your name.')}${group('stage','CAR BACKDROP','Choose the scene behind your car.')}${group('stageTint','BACKDROP COLOR','Retint the scene without moving the car.')}${group('title','RACER TITLE','Choose a title that you have unlocked.')}${group('badge','DISPLAYED BADGE','Choose the badge shown beside your name.')}<label class="profile-favorite-track"><span><b>FAVORITE TRACK</b><small>Shown on your public profile.</small></span><select data-cosmetic-favorite aria-label="Favorite track">${favoriteOptions}</select></label>${podiumChoice}</div><div class="profile-customizer-actions"><span role="status" aria-live="polite"></span><button class="button" type="button" data-save-profile-cosmetics data-account-id="${escapeHtml(accountId)}">Save for everyone</button></div></div></details>`;
  }
  ...
  function ensureStyles(){
    ...
    style.textContent = `
      ...
      .sq-settings-grid .sq-setting-wide{grid-column:1/-1!important}.sq-setting-secret.is-verified{background:rgba(25,78,73,.62)!important;box-shadow:inset 5px 0 #79efad}.sq-setting-secret.is-verified .sq-backup-validation{color:#9ff4bf!important}.sq-setting-secret.is-verified .button{background:#276b60!important;color:#fff!important}.overall-best-line b,.profile-guide b,.profile-result b,.profile-customizer b{font-weight:700!important}
      .overall-entry[class*='cosmetic-stripe-']{border-right:5px solid transparent}.overall-entry.cosmetic-stripe-cyan{border-right-color:#7ee7ff}.overall-entry.cosmetic-stripe-sunset{border-right-color:#ff9a65}.overall-entry.cosmetic-stripe-gold{border-right-color:#ffd56e}.overall-entry.cosmetic-stripe-beta{border-right-color:#ff82d9}.overall-entry.cosmetic-theme-cyan:not(.top-1):not(.top-2):not(.top-3){background:linear-gradient(90deg,#164b68,#263f76)!important}.overall-entry.cosmetic-theme-sunset:not(.top-1):not(.top-2):not(.top-3){background:linear-gradient(90deg,#71473d,#3c3d6d)!important}.overall-entry.cosmetic-theme-forest:not(.top-1):not(.top-2):not(.top-3){background:linear-gradient(90deg,#285448,#2c3e68)!important}.overall-entry.cosmetic-theme-podium:not(.top-1):not(.top-2):not(.top-3){background:linear-gradient(90deg,#715d25,#3c416d)!important}.overall-entry.cosmetic-theme-beta:not(.top-1):not(.top-2):not(.top-3){background:linear-gradient(90deg,#5c3972,#245578)!important}.overall-profile-card.cosmetic-theme-cyan{background:linear-gradient(145deg,#244f78,#24366e)!important}.overall-profile-card.cosmetic-theme-sunset{background:linear-gradient(145deg,#70483f,#303b75)!important}.overall-profile-card.cosmetic-theme-forest{background:linear-gradient(145deg,#285348,#29396d)!important}.overall-profile-card.cosmetic-theme-podium{background:linear-gradient(145deg,#6e5b29,#343b70)!important}.overall-profile-card.cosmetic-theme-beta{background:linear-gradient(145deg,#5d3b76,#285373)!important}.overall-profile-card.cosmetic-stage-grid .profile-car-column>.overall-car-model{background:linear-gradient(165deg,#263d73 0 62%,#f1d66b 63% 66%,#172653 67%)!important}.overall-profile-card.cosmetic-stage-night .profile-car-column>.overall-car-model{background:radial-gradient(circle at 50% 25%,#31588b,#0b1535 72%)!important}.overall-profile-card.cosmetic-stage-podium .profile-car-column>.overall-car-model{background:radial-gradient(circle at 50% 20%,#a98a38,#232d5e 68%)!important}.sq-plain-racer-themes .overall-entry[class*='cosmetic-']{border-right-color:transparent!important}.sq-plain-racer-themes .overall-entry[class*='cosmetic-theme-']:not(.top-1):not(.top-2):not(.top-3){background:#2d3b68!important}.sq-plain-racer-themes .overall-profile-card[class*='cosmetic-theme-']{background:#2d437e!important}.sq-plain-racer-themes .overall-profile-card[class*='cosmetic-stage-'] .profile-car-column>.overall-car-model{background:#172653!important}
      .profile-customizer{margin-top:12px;background:#142653;border-left:5px solid #7ee7ff}.profile-customizer>summary{display:flex;align-items:center;justify-content:space-between;min-height:46px;padding:8px 14px;color:#fff;font-size:18px;cursor:pointer;list-style:none}.profile-customizer>summary::after{content:'+';color:#7ee7ff;font-size:25px}.profile-customizer[open]>summary::after{content:'−'}.profile-customizer-body{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;padding:12px;background:#10204a}.profile-customizer-body>p,.profile-customizer-actions{grid-column:1/-1}.profile-customizer-body>p{margin:0;color:#b9cde8;font-size:14px}.profile-cosmetic-group{min-width:0;margin:0;padding:9px;border:1px solid rgba(126,231,255,.2)}.profile-cosmetic-group legend{padding:0 6px;color:#8feeff;font-size:11px;letter-spacing:1px}.profile-cosmetic-group>div{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:6px}.profile-cosmetic-choice{display:grid;grid-template-columns:20px minmax(0,1fr);grid-template-rows:auto auto;align-items:center;min-height:48px;padding:6px;border:1px solid transparent;background:#1f376f;color:#fff;text-align:left;font:inherit;cursor:pointer}.profile-cosmetic-choice i{grid-row:1/3;width:16px;height:30px;background:#7ee7ff}.profile-cosmetic-choice b{font-size:13px}.profile-cosmetic-choice small{color:#9eb4d2;font-size:9px}.profile-cosmetic-choice.selected{border-color:#fff;background:#31528f}.profile-cosmetic-choice:disabled{opacity:.42;cursor:not-allowed}.profile-cosmetic-choice.cosmetic-theme-sunset i,.profile-cosmetic-choice.cosmetic-stripe-sunset i{background:#ff9a65}.profile-cosmetic-choice.cosmetic-theme-forest i{background:#6fd4a3}.profile-cosmetic-choice.cosmetic-theme-podium i,.profile-cosmetic-choice.cosmetic-stage-podium i,.profile-cosmetic-choice.cosmetic-stripe-gold i{background:#ffd56e}.profile-cosmetic-choice.cosmetic-theme-beta i,.profile-cosmetic-choice.cosmetic-stripe-beta i{background:#ff82d9}.profile-cosmetic-choice.cosmetic-stage-night i{background:#182652}.profile-customizer-actions{display:flex;align-items:center;justify-content:flex-end;gap:12px}.profile-customizer-actions span{margin-right:auto;color:#9ff4bf}.profile-customizer-actions .button{min-width:130px}.profile-beta-badge{font-weight:700!important}
      .overall-entry[class*='cosmetic-stripe-']{--cosmetic-a:#283b70;--cosmetic-b:#172850;--cosmetic-glow:#7ee7ff;isolation:isolate;overflow:hidden;border-right:0}.overall-entry[class*='cosmetic-stripe-']>*{position:relative;z-index:2}.overall-entry[class*='cosmetic-stripe-']::before{content:'';position:absolute;z-index:1;inset:0;pointer-events:none;opacity:.82}.overall-entry.cosmetic-theme-classic{--cosmetic-a:#2d3b68;--cosmetic-b:#1a2958;--cosmetic-glow:#7ee7ff}.overall-entry.cosmetic-theme-cyan{--cosmetic-a:#086782;--cosmetic-b:#263d78;--cosmetic-glow:#7ef5ff}.overall-entry.cosmetic-theme-ocean{--cosmetic-a:#075b79;--cosmetic-b:#0b315f;--cosmetic-glow:#5fe8d0}.overall-entry.cosmetic-theme-sunset{--cosmetic-a:#a34f32;--cosmetic-b:#44366f;--cosmetic-glow:#ffbd73}.overall-entry.cosmetic-theme-forest{--cosmetic-a:#176549;--cosmetic-b:#203963;--cosmetic-glow:#79efad}.overall-entry.cosmetic-theme-ember{--cosmetic-a:#9b2f25;--cosmetic-b:#512951;--cosmetic-glow:#ff9d5d}.overall-entry.cosmetic-theme-podium{--cosmetic-a:#9a761b;--cosmetic-b:#3d396a;--cosmetic-glow:#ffe27a}.overall-entry.cosmetic-theme-beta{--cosmetic-a:#92377f;--cosmetic-b:#175f86;--cosmetic-glow:#ff82d9}.overall-entry[class*='cosmetic-theme-']:not(.top-1):not(.top-2):not(.top-3),.overall-entry.cosmetic-podium-override[class*='cosmetic-theme-']{background:linear-gradient(104deg,var(--cosmetic-a),var(--cosmetic-b) 72%)!important;box-shadow:inset 0 0 0 1px color-mix(in srgb,var(--cosmetic-glow) 42%,transparent),inset 0 0 30px rgba(0,0,0,.18)!important}.overall-entry.cosmetic-stripe-standard::before{background:linear-gradient(90deg,transparent 68%,color-mix(in srgb,var(--cosmetic-glow) 16%,transparent))}.overall-entry.cosmetic-stripe-cyan::before{background:repeating-linear-gradient(112deg,transparent 0 32px,color-mix(in srgb,var(--cosmetic-glow) 30%,transparent) 33px 37px,transparent 38px 58px)}.overall-entry.cosmetic-stripe-chevron::before{background:linear-gradient(135deg,transparent 0 74%,color-mix(in srgb,var(--cosmetic-glow) 30%,transparent) 74% 78%,transparent 78%),repeating-linear-gradient(135deg,transparent 0 32px,color-mix(in srgb,var(--cosmetic-glow) 18%,transparent) 33px 43px,transparent 44px 65px)}.overall-entry.cosmetic-stripe-sunset::before{background:linear-gradient(100deg,transparent 0 36%,rgba(255,206,111,.23) 36% 39%,transparent 39% 56%,rgba(255,112,92,.3) 56% 60%,transparent 60%)}.overall-entry.cosmetic-stripe-grid::before{background:linear-gradient(color-mix(in srgb,var(--cosmetic-glow) 18%,transparent) 1px,transparent 1px),linear-gradient(90deg,color-mix(in srgb,var(--cosmetic-glow) 18%,transparent) 1px,transparent 1px);background-size:25px 25px;mask-image:linear-gradient(90deg,transparent,#000 25%,#000)}.overall-entry.cosmetic-stripe-circuit::before{background:radial-gradient(circle at 72% 30%,var(--cosmetic-glow) 0 3px,transparent 4px),radial-gradient(circle at 88% 72%,var(--cosmetic-glow) 0 3px,transparent 4px),linear-gradient(90deg,transparent 60%,color-mix(in srgb,var(--cosmetic-glow) 40%,transparent) 60% 60.5%,transparent 60.5%),linear-gradient(155deg,transparent 73%,color-mix(in srgb,var(--cosmetic-glow) 35%,transparent) 73% 74%,transparent 74%)}.overall-entry.cosmetic-stripe-gold::before{background:linear-gradient(110deg,transparent 0 56%,rgba(255,238,146,.42) 56% 59%,transparent 59% 68%,rgba(255,200,62,.28) 68% 75%,transparent 75%)}.overall-entry.cosmetic-stripe-beta::before{background:repeating-linear-gradient(125deg,transparent 0 34px,rgba(255,130,217,.3) 35px 43px,rgba(126,231,255,.24) 44px 52px,transparent 53px 80px)}
      .cosmetic-stage-garage .overall-car-model{background:linear-gradient(145deg,#1d2a55,#111c3c)!important}.cosmetic-stage-slate .overall-car-model{background:#405276!important;box-shadow:inset 0 -8px rgba(8,18,40,.2)}.cosmetic-stage-aqua .overall-car-model{background:radial-gradient(circle at 50% 42%,#42d5d6,#123c68 66%,#101d43)!important}.cosmetic-stage-grid .overall-car-model{background:linear-gradient(165deg,#284786 0 60%,#f5db69 61% 64%,#172653 65%)!important}.cosmetic-stage-grid .overall-car-model::after{content:'';position:absolute;inset:64% 0 0;background:repeating-linear-gradient(90deg,transparent 0 20px,rgba(255,255,255,.24) 21px 25px);z-index:0}.cosmetic-stage-horizon .overall-car-model{background:linear-gradient(180deg,#ee8b67 0,#675080 47%,#21355f 48% 72%,#0f1d42 73%)!important}.cosmetic-stage-night .overall-car-model{background:radial-gradient(circle at 18% 20%,#ffe78c 0 2px,transparent 3px),radial-gradient(circle at 78% 28%,#79e8ff 0 2px,transparent 3px),linear-gradient(160deg,#223f72,#09142f 72%)!important;background-size:44px 38px,58px 46px,auto!important}.cosmetic-stage-podium .overall-car-model{background:radial-gradient(circle at 50% 18%,#fff0a3,#a9852f 18%,#30396f 62%,#17234d)!important}.cosmetic-stage-grid .overall-car-model>img,.cosmetic-stage-horizon .overall-car-model>img,.cosmetic-stage-night .overall-car-model>img,.cosmetic-stage-podium .overall-car-model>img{z-index:1}
      .overall-profile-card[class*='cosmetic-theme-']{--profile-a:#2d437e;--profile-b:#172953;--profile-accent:#7ee7ff;background:linear-gradient(145deg,var(--profile-a),var(--profile-b))!important;border:1px solid color-mix(in srgb,var(--profile-accent) 45%,transparent)}.overall-profile-card.cosmetic-theme-cyan{--profile-a:#116b87;--profile-b:#283778;--profile-accent:#7ef5ff}.overall-profile-card.cosmetic-theme-ocean{--profile-a:#08627c;--profile-b:#102c58;--profile-accent:#5fe8d0}.overall-profile-card.cosmetic-theme-sunset{--profile-a:#9a4d39;--profile-b:#39346e;--profile-accent:#ffbd73}.overall-profile-card.cosmetic-theme-forest{--profile-a:#22654c;--profile-b:#25345f;--profile-accent:#79efad}.overall-profile-card.cosmetic-theme-ember{--profile-a:#8f352c;--profile-b:#43294f;--profile-accent:#ff9d5d}.overall-profile-card.cosmetic-theme-podium{--profile-a:#806724;--profile-b:#303765;--profile-accent:#ffe27a}.overall-profile-card.cosmetic-theme-beta{--profile-a:#7d3d82;--profile-b:#1e5278;--profile-accent:#ff82d9}.overall-profile-card[class*='cosmetic-theme-'] .profile-hero,.overall-profile-card[class*='cosmetic-theme-'] .profile-customizer{border-color:var(--profile-accent)}.overall-profile-card[class*='cosmetic-theme-'] .profile-kicker{color:var(--profile-accent)}
      .profile-customizer{margin-top:16px;border-left-width:7px;box-shadow:0 8px 24px rgba(4,10,29,.25)}.profile-customizer>summary{min-height:58px;padding:10px 16px;background:linear-gradient(100deg,rgba(126,231,255,.16),transparent);gap:14px}.profile-customizer>summary>span{display:flex;flex-direction:column;gap:3px}.profile-customizer>summary b{font-size:18px;letter-spacing:.8px}.profile-customizer>summary small{color:#b9cde8;font-size:12px}.profile-customizer>summary>em{margin-left:auto;padding:7px 10px;background:#7ee7ff;color:#10214b;font-size:12px;font-style:normal}.profile-customizer>summary::after{margin-left:0}.profile-customizer-intro{grid-column:1/-1;display:flex;align-items:center;gap:12px;padding:11px 13px;background:#1b3570;border-left:4px solid #7ee7ff}.profile-customizer-intro b{font-size:18px;color:#fff}.profile-customizer-intro span{color:#c6daf2;font-size:13px}.profile-cosmetic-group{padding:12px;background:rgba(9,21,53,.35);border:1px solid rgba(126,231,255,.28)}.profile-cosmetic-group>p{min-height:30px;margin:0 0 9px;color:#b9cde8;font-size:12px;line-height:1.25}.profile-cosmetic-group legend{font-size:12px;font-weight:700}.profile-cosmetic-group>div{grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.profile-cosmetic-choice{grid-template-columns:74px minmax(0,1fr);min-height:62px;padding:7px;background:#1b3268}.profile-cosmetic-preview{grid-row:1/3;position:relative;width:66px;height:42px;overflow:hidden;background:linear-gradient(105deg,#2f467e,#182958);clip-path:polygon(5px 0,100% 0,calc(100% - 5px) 100%,0 100%)}.profile-cosmetic-preview i{position:absolute;inset:0;width:auto;height:auto;background:linear-gradient(105deg,transparent 20%,#7ee7ff 21% 25%,transparent 26% 55%,rgba(126,231,255,.35) 56% 66%,transparent 67%)}.profile-cosmetic-choice b{font-size:14px}.profile-cosmetic-choice small{font-size:10px}.profile-cosmetic-choice.selected{border-color:#fff;background:#315795;box-shadow:0 0 0 2px rgba(126,231,255,.25)}.profile-cosmetic-choice:disabled{filter:grayscale(.45)}.profile-cosmetic-choice.cosmetic-theme-sunset .profile-cosmetic-preview,.profile-cosmetic-choice.cosmetic-stage-horizon .profile-cosmetic-preview{background:linear-gradient(105deg,#af5237,#42346f)}.profile-cosmetic-choice.cosmetic-theme-forest .profile-cosmetic-preview{background:linear-gradient(105deg,#19694b,#24345e)}.profile-cosmetic-choice.cosmetic-theme-ember .profile-cosmetic-preview{background:linear-gradient(105deg,#a63828,#482748)}.profile-cosmetic-choice.cosmetic-theme-ocean .profile-cosmetic-preview,.profile-cosmetic-choice.cosmetic-stage-aqua .profile-cosmetic-preview{background:linear-gradient(105deg,#08728a,#10325f)}.profile-cosmetic-choice.cosmetic-theme-podium .profile-cosmetic-preview,.profile-cosmetic-choice.cosmetic-stage-podium .profile-cosmetic-preview,.profile-cosmetic-choice.cosmetic-stripe-gold .profile-cosmetic-preview{background:linear-gradient(105deg,#a47c1d,#3d396a)}.profile-cosmetic-choice.cosmetic-theme-beta .profile-cosmetic-preview,.profile-cosmetic-choice.cosmetic-stripe-beta .profile-cosmetic-preview{background:linear-gradient(105deg,#a23b8d,#176588)}.profile-cosmetic-choice.cosmetic-stage-slate .profile-cosmetic-preview{background:#405276}.profile-cosmetic-choice.cosmetic-stage-night .profile-cosmetic-preview{background:radial-gradient(circle,#78efff 0 2px,transparent 3px),#101c42;background-size:18px 18px}.profile-cosmetic-choice.cosmetic-stripe-chevron .profile-cosmetic-preview i{background:repeating-linear-gradient(135deg,transparent 0 10px,rgba(126,231,255,.65) 11px 15px,transparent 16px 24px)}.profile-cosmetic-choice.cosmetic-stripe-grid .profile-cosmetic-preview i{background:linear-gradient(rgba(126,231,255,.5) 1px,transparent 1px),linear-gradient(90deg,rgba(126,231,255,.5) 1px,transparent 1px);background-size:10px 10px}.profile-cosmetic-choice.cosmetic-stripe-circuit .profile-cosmetic-preview i{background:radial-gradient(circle at 70% 25%,#7ee7ff 0 2px,transparent 3px),linear-gradient(160deg,transparent 55%,#7ee7ff 56% 59%,transparent 60%)}.profile-cosmetic-podium{grid-column:1/-1;display:flex;align-items:center;gap:14px;padding:11px 13px;background:#1a2f64;border-left:4px solid #ffe27a}.profile-cosmetic-podium>div{display:flex;flex-direction:column;gap:3px}.profile-cosmetic-podium small{color:#b9cde8;font-size:12px}.profile-cosmetic-podium .button{margin-left:auto;min-width:180px}.profile-cosmetic-podium .button.selected{background:#ffe27a!important;color:#17234d!important}.profile-customizer-actions{min-height:52px;padding-top:4px}.profile-customizer-actions .button{min-width:190px;background:#7ee7ff!important;color:#10214b!important}.sq-reduced-effects .overall-entry.cosmetic-stripe-beta::before{animation:none!important}
      .sq-plain-racer-themes .overall-entry[class*='cosmetic-stripe-']::before{display:none!important}.sq-plain-racer-themes .overall-entry[class*='cosmetic-theme-']{background:var(--rank-surface)!important;box-shadow:none!important}.sq-plain-racer-themes .overall-profile-card[class*='cosmetic-theme-']{--profile-a:#2d437e;--profile-b:#2d437e;--profile-accent:#7ee7ff}.sq-plain-racer-themes .overall-profile-card[class*='cosmetic-stage-'] .overall-car-model{background:#172653!important}
      .leaderboard-ui>.container>button.main.sq-racer-cosmetic{--cosmetic-a:#2d3b68;--cosmetic-b:#192958;--cosmetic-glow:#7ee7ff;position:relative;isolation:isolate;overflow:hidden}.leaderboard-ui>.container>button.main.sq-racer-cosmetic>*{position:relative;z-index:2}.leaderboard-ui>.container>button.main.sq-racer-cosmetic::before{content:'';position:absolute;z-index:1;inset:0;pointer-events:none;opacity:.72}.leaderboard-ui>.container>button.main.sq-racer-cosmetic.cosmetic-theme-cyan{--cosmetic-a:#086782;--cosmetic-b:#263d78;--cosmetic-glow:#7ef5ff}.leaderboard-ui>.container>button.main.sq-racer-cosmetic.cosmetic-theme-ocean{--cosmetic-a:#075b79;--cosmetic-b:#0b315f;--cosmetic-glow:#5fe8d0}.leaderboard-ui>.container>button.main.sq-racer-cosmetic.cosmetic-theme-sunset{--cosmetic-a:#a34f32;--cosmetic-b:#44366f;--cosmetic-glow:#ffbd73}.leaderboard-ui>.container>button.main.sq-racer-cosmetic.cosmetic-theme-forest{--cosmetic-a:#176549;--cosmetic-b:#203963;--cosmetic-glow:#79efad}.leaderboard-ui>.container>button.main.sq-racer-cosmetic.cosmetic-theme-ember{--cosmetic-a:#9b2f25;--cosmetic-b:#512951;--cosmetic-glow:#ff9d5d}.leaderboard-ui>.container>button.main.sq-racer-cosmetic.cosmetic-theme-podium{--cosmetic-a:#9a761b;--cosmetic-b:#3d396a;--cosmetic-glow:#ffe27a}.leaderboard-ui>.container>button.main.sq-racer-cosmetic.cosmetic-theme-beta{--cosmetic-a:#92377f;--cosmetic-b:#175f86;--cosmetic-glow:#ff82d9}.leaderboard-ui>.container>button.main.sq-racer-cosmetic:not(.sq-racer-top),.leaderboard-ui>.container>button.main.sq-racer-cosmetic.cosmetic-podium-override{background:linear-gradient(104deg,var(--cosmetic-a),var(--cosmetic-b) 72%)!important}.leaderboard-ui>.container>button.main.sq-racer-cosmetic.sq-racer-top:not(.cosmetic-podium-override)::before{display:none}.leaderboard-ui>.container>button.main.sq-racer-cosmetic.cosmetic-stripe-cyan::before{background:repeating-linear-gradient(112deg,transparent 0 32px,color-mix(in srgb,var(--cosmetic-glow) 30%,transparent) 33px 37px,transparent 38px 58px)}.leaderboard-ui>.container>button.main.sq-racer-cosmetic.cosmetic-stripe-chevron::before{background:repeating-linear-gradient(135deg,transparent 0 28px,color-mix(in srgb,var(--cosmetic-glow) 22%,transparent) 29px 40px,transparent 41px 62px)}.leaderboard-ui>.container>button.main.sq-racer-cosmetic.cosmetic-stripe-sunset::before{background:linear-gradient(100deg,transparent 0 36%,rgba(255,206,111,.25) 36% 40%,transparent 40% 58%,rgba(255,112,92,.3) 58% 63%,transparent 63%)}.leaderboard-ui>.container>button.main.sq-racer-cosmetic.cosmetic-stripe-grid::before{background:linear-gradient(color-mix(in srgb,var(--cosmetic-glow) 18%,transparent) 1px,transparent 1px),linear-gradient(90deg,color-mix(in srgb,var(--cosmetic-glow) 18%,transparent) 1px,transparent 1px);background-size:24px 24px}.leaderboard-ui>.container>button.main.sq-racer-cosmetic.cosmetic-stripe-circuit::before{background:radial-gradient(circle at 72% 30%,var(--cosmetic-glow) 0 3px,transparent 4px),radial-gradient(circle at 88% 72%,var(--cosmetic-glow) 0 3px,transparent 4px),linear-gradient(155deg,transparent 72%,color-mix(in srgb,var(--cosmetic-glow) 35%,transparent) 73% 74%,transparent 75%)}.leaderboard-ui>.container>button.main.sq-racer-cosmetic.cosmetic-stripe-gold::before{background:linear-gradient(110deg,transparent 0 55%,rgba(255,238,146,.45) 56% 60%,transparent 61% 69%,rgba(255,200,62,.3) 70% 77%,transparent 78%)}.leaderboard-ui>.container>button.main.sq-racer-cosmetic.cosmetic-stripe-beta::before{background:repeating-linear-gradient(125deg,transparent 0 34px,rgba(255,130,217,.32) 35px 43px,rgba(126,231,255,.26) 44px 52px,transparent 53px 80px)}.sq-plain-racer-themes .leaderboard-ui>.container>button.main.sq-racer-cosmetic{background:revert-layer!important}.sq-plain-racer-themes .leaderboard-ui>.container>button.main.sq-racer-cosmetic::before{display:none!important}
      @media(max-width:700px){.profile-customizer-body{grid-template-columns:1fr}.profile-cosmetic-group>div{grid-template-columns:repeat(2,minmax(0,1fr))}.profile-cosmetic-choice{grid-template-columns:56px minmax(0,1fr)}.profile-cosmetic-preview{width:49px}.profile-cosmetic-podium{align-items:flex-start;flex-direction:column}.profile-cosmetic-podium .button{width:100%;margin:0}.sq-setting-secret{grid-template-columns:1fr!important}.sq-setting-secret .sq-secret-controls,.sq-setting-secret .sq-backup-validation{grid-column:1!important}}
      .profile-cosmetic-group.kind-stripe{grid-column:1/-1}.profile-cosmetic-group.kind-stripe>div{grid-template-columns:repeat(2,minmax(0,1fr))}.profile-cosmetic-group.kind-stripe .profile-cosmetic-choice{grid-template-columns:126px minmax(0,1fr);min-height:78px}.profile-row-preview{display:grid;grid-template-rows:repeat(3,16px);gap:3px;width:116px}.profile-row-preview i{--row-base:linear-gradient(105deg,#344b82,#1a2b5c);display:flex;align-items:center;justify-content:flex-start;box-sizing:border-box;height:16px;padding:0 6px;overflow:hidden;border:1px solid rgba(255,255,255,.18);border-left:4px solid rgba(126,231,255,.55);border-radius:1px;background:var(--row-base);color:#f7fbff;font:normal 9px/1 ForcedSquare,Arial,sans-serif;letter-spacing:.4px;text-shadow:0 1px 1px #07122c}.profile-row-preview .preview-first{--row-base:linear-gradient(105deg,#9c7416,#4b3a18);border-left-color:#ffe27a;color:#fff6c8}.profile-row-preview .preview-other{--row-base:linear-gradient(105deg,#344b82,#1a2b5c)}.profile-row-preview .preview-you{--row-base:linear-gradient(105deg,#14647d,#243e78);border-left-color:#7ee7ff;color:#dffbff}.profile-cosmetic-choice.cosmetic-stripe-cyan .profile-row-preview i{background:repeating-linear-gradient(112deg,transparent 0 20px,rgba(126,245,255,.32) 21px 24px,transparent 25px 38px),var(--row-base)}.profile-cosmetic-choice.cosmetic-stripe-chevron .profile-row-preview i{background:repeating-linear-gradient(135deg,transparent 0 14px,rgba(126,231,255,.34) 15px 21px,transparent 22px 31px),var(--row-base)}.profile-cosmetic-choice.cosmetic-stripe-sunset .profile-row-preview i{background:linear-gradient(100deg,transparent 0 34%,rgba(255,205,111,.38) 35% 42%,transparent 43% 60%,rgba(255,108,88,.4) 61% 70%,transparent 71%),var(--row-base)}.profile-cosmetic-choice.cosmetic-stripe-grid .profile-row-preview i{background:linear-gradient(rgba(126,231,255,.22) 1px,transparent 1px),linear-gradient(90deg,rgba(126,231,255,.22) 1px,transparent 1px),var(--row-base);background-size:10px 10px,10px 10px,auto}.profile-cosmetic-choice.cosmetic-stripe-circuit .profile-row-preview i{background:radial-gradient(circle at 74% 25%,#7ee7ff 0 2px,transparent 3px),linear-gradient(158deg,transparent 60%,rgba(126,231,255,.38) 61% 64%,transparent 65%),var(--row-base)}.profile-cosmetic-choice.cosmetic-stripe-gold .profile-row-preview i{background:linear-gradient(110deg,transparent 0 45%,rgba(255,238,146,.5) 46% 53%,transparent 54% 64%,rgba(255,200,62,.36) 65% 76%,transparent 77%),var(--row-base)}.profile-cosmetic-choice.cosmetic-stripe-beta .profile-row-preview i{background:repeating-linear-gradient(125deg,transparent 0 17px,rgba(255,130,217,.36) 18px 24px,rgba(126,231,255,.3) 25px 31px,transparent 32px 45px),var(--row-base)}
      .overall-profile-card.cosmetic-podium-override .profile-row-preview .preview-first{--row-base:linear-gradient(105deg,var(--profile-a,#344b82),var(--profile-b,#1a2b5c));border-left-color:var(--profile-accent,#7ee7ff);color:#fff}
      @media(max-width:700px){.profile-cosmetic-group.kind-stripe>div{grid-template-columns:1fr}.profile-cosmetic-group.kind-stripe .profile-cosmetic-choice{grid-template-columns:110px minmax(0,1fr)}.profile-row-preview{width:100px}}
      .overall-category-control{display:grid;grid-template-columns:minmax(0,1fr) 44px;gap:7px;align-items:stretch}.overall-category-info{position:relative;display:flex;align-items:center;justify-content:center;min-width:44px;border:1px solid rgba(126,231,255,.45);border-radius:2px;background:#10204a;color:#8fefff;font:24px ForcedSquare,Arial,sans-serif;cursor:help}.overall-category-info:hover,.overall-category-info:focus-visible{background:#7ee7ff;color:#10204a;outline:2px solid #fff;outline-offset:2px}.overall-category-info::after{content:attr(data-tip);position:absolute;z-index:30;right:0;bottom:calc(100% + 8px);box-sizing:border-box;width:min(330px,70vw);padding:10px 12px;border:1px solid #7ee7ff;background:#0d193c;color:#fff;font:14px/1.35 ForcedSquare,Arial,sans-serif;text-align:left;box-shadow:0 8px 24px rgba(2,8,25,.45);opacity:0;pointer-events:none;transform:translateY(4px);transition:opacity .14s ease,transform .14s ease}.overall-category-info:hover::after,.overall-category-info:focus-visible::after{opacity:1;transform:translateY(0)}
      .overall-entry.cosmetic-theme-ice{--cosmetic-a:#3b86a8;--cosmetic-b:#274989;--cosmetic-glow:#d8fbff}.overall-entry.cosmetic-theme-mono{--cosmetic-a:#555e70;--cosmetic-b:#20293d;--cosmetic-glow:#f4f7ff}.overall-entry.cosmetic-theme-neon{--cosmetic-a:#036f71;--cosmetic-b:#7a245f;--cosmetic-glow:#6dffda}.overall-entry.cosmetic-theme-crimson{--cosmetic-a:#a51f3f;--cosmetic-b:#3f234c;--cosmetic-glow:#ff8aa0}.overall-profile-card.cosmetic-theme-ice{--profile-a:#4c91ad;--profile-b:#29437e;--profile-accent:#d8fbff}.overall-profile-card.cosmetic-theme-mono{--profile-a:#535d70;--profile-b:#20293d;--profile-accent:#f4f7ff}.overall-profile-card.cosmetic-theme-neon{--profile-a:#087776;--profile-b:#74265f;--profile-accent:#6dffda}.overall-profile-card.cosmetic-theme-crimson{--profile-a:#941f3c;--profile-b:#3d244c;--profile-accent:#ff8aa0}
      .overall-entry.cosmetic-stripe-apex::before,.leaderboard-ui>.container>button.main.sq-racer-cosmetic.cosmetic-stripe-apex::before{background:linear-gradient(118deg,transparent 0 58%,color-mix(in srgb,var(--cosmetic-glow) 52%,transparent) 59% 63%,transparent 64%),linear-gradient(118deg,transparent 0 72%,color-mix(in srgb,var(--cosmetic-glow) 28%,transparent) 73% 76%,transparent 77%)}.overall-entry.cosmetic-stripe-split::before,.leaderboard-ui>.container>button.main.sq-racer-cosmetic.cosmetic-stripe-split::before{background:linear-gradient(105deg,transparent 0 47%,color-mix(in srgb,var(--cosmetic-glow) 42%,transparent) 48% 52%,rgba(255,255,255,.08) 53% 70%,transparent 71%)}.overall-entry.cosmetic-stripe-scan::before,.leaderboard-ui>.container>button.main.sq-racer-cosmetic.cosmetic-stripe-scan::before{background:repeating-linear-gradient(0deg,transparent 0 5px,color-mix(in srgb,var(--cosmetic-glow) 22%,transparent) 6px 7px)}.overall-entry.cosmetic-stripe-blocks::before,.leaderboard-ui>.container>button.main.sq-racer-cosmetic.cosmetic-stripe-blocks::before{background:linear-gradient(90deg,transparent 0 54%,color-mix(in srgb,var(--cosmetic-glow) 22%,transparent) 54% 64%,transparent 64% 68%,color-mix(in srgb,var(--cosmetic-glow) 34%,transparent) 68% 81%,transparent 81%)}
      .leaderboard-ui>.container>button.main.sq-racer-cosmetic.cosmetic-theme-ice{--cosmetic-a:#3b86a8;--cosmetic-b:#274989;--cosmetic-glow:#d8fbff}.leaderboard-ui>.container>button.main.sq-racer-cosmetic.cosmetic-theme-mono{--cosmetic-a:#555e70;--cosmetic-b:#20293d;--cosmetic-glow:#f4f7ff}.leaderboard-ui>.container>button.main.sq-racer-cosmetic.cosmetic-theme-neon{--cosmetic-a:#036f71;--cosmetic-b:#7a245f;--cosmetic-glow:#6dffda}.leaderboard-ui>.container>button.main.sq-racer-cosmetic.cosmetic-theme-crimson{--cosmetic-a:#a51f3f;--cosmetic-b:#3f234c;--cosmetic-glow:#ff8aa0}
      .cosmetic-stage-dunes .overall-car-model{background:linear-gradient(180deg,#69b5d0 0 43%,#f1b95d 44% 66%,#a75e35 67% 100%)!important}.cosmetic-stage-storm .overall-car-model{background:radial-gradient(circle at 42% 35%,rgba(134,238,255,.55),transparent 23%),linear-gradient(145deg,#182d62,#101730 70%)!important}.cosmetic-stage-storm .overall-car-model::after{content:'';position:absolute;inset:0;background:linear-gradient(120deg,transparent 0 54%,rgba(142,239,255,.7) 55% 56%,transparent 57% 70%,rgba(142,239,255,.35) 71% 72%,transparent 73%);z-index:0}.cosmetic-stage-dunes .overall-car-model>img,.cosmetic-stage-storm .overall-car-model>img{position:relative;z-index:1}
      .profile-row-showcase{--show-a:var(--profile-a,#2d437e);--show-b:var(--profile-b,#172953);--show-accent:var(--profile-accent,#7ee7ff);grid-column:1/-1;display:grid;gap:7px;padding:12px;border:1px solid color-mix(in srgb,var(--show-accent) 46%,#263a70);background:#0e1a3c}.profile-row-showcase header{display:flex;align-items:end;justify-content:space-between;gap:12px}.profile-row-showcase header b{color:var(--show-accent);font-size:17px}.profile-row-showcase header small{color:#c2d3eb;font-size:12px}.showcase-row{position:relative;isolation:isolate;display:grid;grid-template-columns:58px minmax(0,1fr) auto;align-items:center;min-height:48px;padding:5px 12px;overflow:hidden;border-left:6px solid var(--show-accent);background:linear-gradient(104deg,var(--show-a),var(--show-b) 72%);color:#fff;box-shadow:inset 0 0 0 1px rgba(255,255,255,.15)}.showcase-row>*{position:relative;z-index:2}.showcase-row>strong{font-size:22px}.showcase-row>span{display:flex;flex-direction:column;gap:2px}.showcase-row>span>b{font-size:17px;letter-spacing:.3px}.showcase-row>span>small{color:#d7e4f6;font-size:11px}.showcase-row>em{padding:4px 7px;border:1px solid currentColor;color:var(--show-accent);font-size:11px;font-style:normal}.showcase-row::before{content:'';position:absolute;z-index:1;inset:0;opacity:.82}.showcase-first{--show-a:#a67a18;--show-b:#4c3919;--show-accent:#ffe27a}.showcase-self{outline:2px solid #9ff5ff;outline-offset:-2px}.showcase-self>em{background:#b8f7ff;color:#12355c}.cosmetic-podium-override .showcase-first{--show-a:var(--profile-a,#2d437e);--show-b:var(--profile-b,#172953);--show-accent:var(--profile-accent,#7ee7ff)}.cosmetic-stripe-cyan .showcase-row::before{background:repeating-linear-gradient(112deg,transparent 0 32px,color-mix(in srgb,var(--show-accent) 30%,transparent) 33px 37px,transparent 38px 58px)}.cosmetic-stripe-apex .showcase-row::before{background:linear-gradient(118deg,transparent 0 58%,color-mix(in srgb,var(--show-accent) 52%,transparent) 59% 63%,transparent 64%),linear-gradient(118deg,transparent 0 72%,color-mix(in srgb,var(--show-accent) 28%,transparent) 73% 76%,transparent 77%)}.cosmetic-stripe-chevron .showcase-row::before{background:repeating-linear-gradient(135deg,transparent 0 28px,color-mix(in srgb,var(--show-accent) 25%,transparent) 29px 40px,transparent 41px 62px)}.cosmetic-stripe-sunset .showcase-row::before{background:linear-gradient(100deg,transparent 0 36%,rgba(255,206,111,.34) 36% 40%,transparent 40% 58%,rgba(255,112,92,.38) 58% 63%,transparent 63%)}.cosmetic-stripe-split .showcase-row::before{background:linear-gradient(105deg,transparent 0 47%,color-mix(in srgb,var(--show-accent) 42%,transparent) 48% 52%,rgba(255,255,255,.08) 53% 70%,transparent 71%)}.cosmetic-stripe-grid .showcase-row::before{background:linear-gradient(color-mix(in srgb,var(--show-accent) 18%,transparent) 1px,transparent 1px),linear-gradient(90deg,color-mix(in srgb,var(--show-accent) 18%,transparent) 1px,transparent 1px);background-size:22px 22px}.cosmetic-stripe-circuit .showcase-row::before{background:radial-gradient(circle at 72% 30%,var(--show-accent) 0 3px,transparent 4px),linear-gradient(155deg,transparent 72%,color-mix(in srgb,var(--show-accent) 38%,transparent) 73% 74%,transparent 75%)}.cosmetic-stripe-scan .showcase-row::before{background:repeating-linear-gradient(0deg,transparent 0 5px,color-mix(in srgb,var(--show-accent) 22%,transparent) 6px 7px)}.cosmetic-stripe-blocks .showcase-row::before{background:linear-gradient(90deg,transparent 0 54%,color-mix(in srgb,var(--show-accent) 22%,transparent) 54% 64%,transparent 64% 68%,color-mix(in srgb,var(--show-accent) 34%,transparent) 68% 81%,transparent 81%)}.cosmetic-stripe-gold .showcase-row::before{background:linear-gradient(110deg,transparent 0 55%,rgba(255,238,146,.48) 56% 60%,transparent 61% 69%,rgba(255,200,62,.34) 70% 77%,transparent 78%)}.cosmetic-stripe-beta .showcase-row::before{background:repeating-linear-gradient(125deg,transparent 0 34px,rgba(255,130,217,.36) 35px 43px,rgba(126,231,255,.3) 44px 52px,transparent 53px 80px)}
      .profile-cosmetic-group.kind-stripe .profile-cosmetic-choice{grid-template-columns:60px minmax(0,1fr);min-height:64px}.profile-cosmetic-choice.cosmetic-theme-ice .profile-cosmetic-preview{background:linear-gradient(105deg,#5aa8c0,#304d94)}.profile-cosmetic-choice.cosmetic-theme-mono .profile-cosmetic-preview{background:linear-gradient(105deg,#667080,#222b3e)}.profile-cosmetic-choice.cosmetic-theme-neon .profile-cosmetic-preview{background:linear-gradient(105deg,#078780,#8b2868)}.profile-cosmetic-choice.cosmetic-theme-crimson .profile-cosmetic-preview{background:linear-gradient(105deg,#b52445,#472652)}.profile-cosmetic-choice.cosmetic-stage-dunes .profile-cosmetic-preview{background:linear-gradient(180deg,#69b5d0 0 45%,#f1b95d 46% 70%,#a75e35 71%)}.profile-cosmetic-choice.cosmetic-stage-storm .profile-cosmetic-preview{background:linear-gradient(125deg,#17285b 0 50%,#81eaff 51% 54%,#11172f 55%)}.profile-cosmetic-choice.cosmetic-stripe-apex .profile-cosmetic-preview i{background:linear-gradient(118deg,transparent 0 51%,#8cefff 52% 60%,transparent 61%)}.profile-cosmetic-choice.cosmetic-stripe-split .profile-cosmetic-preview i{background:linear-gradient(105deg,transparent 0 43%,#8cefff 44% 53%,rgba(255,255,255,.18) 54% 72%,transparent 73%)}.profile-cosmetic-choice.cosmetic-stripe-scan .profile-cosmetic-preview i{background:repeating-linear-gradient(0deg,transparent 0 5px,rgba(126,231,255,.7) 6px 7px)}.profile-cosmetic-choice.cosmetic-stripe-blocks .profile-cosmetic-preview i{background:linear-gradient(90deg,transparent 0 35%,rgba(126,231,255,.45) 36% 55%,transparent 56% 63%,rgba(126,231,255,.7) 64% 85%,transparent 86%)}
      .leaderboard-ui .verified-state{display:inline-flex!important;align-items:center;gap:6px;min-height:24px;padding:3px 7px!important;border:2px solid currentColor!important;border-radius:2px!important}.leaderboard-ui .verified-state .sq-integrity-label{font-size:11px!important;font-weight:700;letter-spacing:.7px}.leaderboard-ui .verified-state.verified{color:#72f0a4!important;background:rgba(19,91,56,.28)!important}.leaderboard-ui .verified-state.pending{color:#ffd166!important;background:repeating-linear-gradient(135deg,rgba(126,82,10,.34) 0 6px,rgba(75,51,12,.2) 6px 12px)!important;border-style:dashed!important}.leaderboard-ui .verified-state img{width:18px!important;height:18px!important}.leaderboard-ui .verified-state.pending img{width:20px!important;height:20px!important}
      @media(max-width:1100px){.overall-footer-right .overall-category-control{grid-column:1;grid-row:1}}
      @media(max-width:700px){.profile-row-showcase header{align-items:flex-start;flex-direction:column;gap:3px}.showcase-row{grid-template-columns:42px minmax(0,1fr) auto;padding:5px 8px}.showcase-row>span>b{font-size:15px}.showcase-row>em{font-size:9px}.overall-footer-right .overall-category-control{grid-column:1!important;grid-row:auto!important;grid-template-columns:minmax(0,1fr) 44px}}
      /* Racer Studio v3: shared variables keep every public preview and leaderboard surface consistent. */
      ...
      .overall-entry.cosmetic-finish-solid:not(.top-1):not(.top-2):not(.top-3),.overall-entry.cosmetic-finish-solid.cosmetic-podium-override,.leaderboard-ui button.sq-racer-cosmetic.cosmetic-finish-solid:not(.sq-racer-top),.leaderboard-ui button.sq-racer-cosmetic.cosmetic-finish-solid.cosmetic-podium-override,.cosmetic-finish-solid .showcase-row:not(.showcase-first),.cosmetic-finish-solid.cosmetic-podium-override .showcase-row{background:var(--cosmetic-a,var(--profile-a,#2d437e))!important}.overall-entry.cosmetic-finish-split:not(.top-1):not(.top-2):not(.top-3),.overall-entry.cosmetic-finish-split.cosmetic-podium-override,.leaderboard-ui button.sq-racer-cosmetic.cosmetic-finish-split:not(.sq-racer-top),.leaderboard-ui button.sq-racer-cosmetic.cosmetic-finish-split.cosmetic-podium-override,.cosmetic-finish-split .showcase-row:not(.showcase-first),.cosmetic-finish-split.cosmetic-podium-override .showcase-row{background:linear-gradient(110deg,var(--cosmetic-a,var(--profile-a,#2d437e)) 0 48%,var(--cosmetic-b,var(--profile-b,#172953)) 49%)!important}.overall-entry.cosmetic-finish-gloss:not(.top-1):not(.top-2):not(.top-3),.overall-entry.cosmetic-finish-gloss.cosmetic-podium-override,.leaderboard-ui button.sq-racer-cosmetic.cosmetic-finish-gloss:not(.sq-racer-top),.leaderboard-ui button.sq-racer-cosmetic.cosmetic-finish-gloss.cosmetic-podium-override,.cosmetic-finish-gloss .showcase-row:not(.showcase-first),.cosmetic-finish-gloss.cosmetic-podium-override .showcase-row{background:linear-gradient(165deg,rgba(255,255,255,.23) 0 18%,transparent 19%),linear-gradient(104deg,var(--cosmetic-a,var(--profile-a,#2d437e)),var(--cosmetic-b,var(--profile-b,#172953)) 72%)!important}.overall-entry.cosmetic-finish-carbon:not(.top-1):not(.top-2):not(.top-3),.overall-entry.cosmetic-finish-carbon.cosmetic-podium-override,.leaderboard-ui button.sq-racer-cosmetic.cosmetic-finish-carbon:not(.sq-racer-top),.leaderboard-ui button.sq-racer-cosmetic.cosmetic-finish-carbon.cosmetic-podium-override,.cosmetic-finish-carbon .showcase-row:not(.showcase-first),.cosmetic-finish-carbon.cosmetic-podium-override .showcase-row{background:linear-gradient(45deg,rgba(255,255,255,.06) 25%,transparent 25% 75%,rgba(255,255,255,.06) 75%),linear-gradient(135deg,rgba(0,0,0,.12) 25%,transparent 25% 75%,rgba(0,0,0,.12) 75%),linear-gradient(104deg,var(--cosmetic-a,var(--profile-a,#2d437e)),var(--cosmetic-b,var(--profile-b,#172953)))!important;background-size:12px 12px,12px 12px,auto!important}.overall-entry.cosmetic-finish-horizon:not(.top-1):not(.top-2):not(.top-3),.overall-entry.cosmetic-finish-horizon.cosmetic-podium-override,.leaderboard-ui button.sq-racer-cosmetic.cosmetic-finish-horizon:not(.sq-racer-top),.leaderboard-ui button.sq-racer-cosmetic.cosmetic-finish-horizon.cosmetic-podium-override,.cosmetic-finish-horizon .showcase-row:not(.showcase-first),.cosmetic-finish-horizon.cosmetic-podium-override .showcase-row{background:linear-gradient(180deg,var(--cosmetic-a,var(--profile-a,#2d437e)) 0 45%,color-mix(in srgb,var(--cosmetic-glow,var(--profile-accent,#7ee7ff)) 45%,transparent) 46% 50%,var(--cosmetic-b,var(--profile-b,#172953)) 51%)!important}
      .overall-entry .overall-name,.leaderboard-ui button.main .name{position:relative}.overall-entry[class*='cosmetic-emblem-'] .overall-name::before,.leaderboard-ui button.main[class*='cosmetic-emblem-'] .name::before{position:absolute;right:2%;top:50%;z-index:0;transform:translateY(-50%);color:var(--cosmetic-glow,#7ee7ff);font:46px/1 ForcedSquare,Arial,sans-serif;opacity:.13;pointer-events:none}.cosmetic-emblem-bolt .overall-name::before,.leaderboard-ui .cosmetic-emblem-bolt .name::before,.cosmetic-emblem-bolt .showcase-row::after{content:'⚡'}.cosmetic-emblem-star .overall-name::before,.leaderboard-ui .cosmetic-emblem-star .name::before,.cosmetic-emblem-star .showcase-row::after{content:'★'}.cosmetic-emblem-flag .overall-name::before,.leaderboard-ui .cosmetic-emblem-flag .name::before,.cosmetic-emblem-flag .showcase-row::after{content:'⚑'}.cosmetic-emblem-flame .overall-name::before,.leaderboard-ui .cosmetic-emblem-flame .name::before,.cosmetic-emblem-flame .showcase-row::after{content:'▲'}.cosmetic-emblem-crown .overall-name::before,.leaderboard-ui .cosmetic-emblem-crown .name::before,.cosmetic-emblem-crown .showcase-row::after{content:'♛'}.showcase-row::after{position:absolute;right:12%;top:50%;z-index:1;transform:translateY(-50%);color:var(--show-accent);font:38px/1 ForcedSquare,Arial,sans-serif;opacity:.14}
      .overall-racer-title,.profile-racer-title{display:inline-flex;padding:2px 6px;border:1px solid var(--cosmetic-glow,var(--profile-accent,#7ee7ff));color:var(--cosmetic-glow,var(--profile-accent,#9feaff));font-size:11px;letter-spacing:.7px}.profile-identity-extras{display:flex;flex-wrap:wrap;align-items:center;gap:7px;margin:-3px 0 10px}.profile-favorite-chip{min-height:30px;padding:4px 9px;border:1px solid color-mix(in srgb,var(--profile-accent,#7ee7ff) 55%,transparent);background:#142653;color:#fff;font:13px ForcedSquare,Arial,sans-serif;cursor:pointer}.profile-favorite-chip:hover,.profile-favorite-chip:focus-visible{background:var(--profile-accent,#7ee7ff);color:#10204a;outline:2px solid #fff}
      .overall-profile-card[class*='cosmetic-']{border-left:7px solid var(--profile-accent,#7ee7ff)}.overall-profile-card[class*='cosmetic-stripe-'] .profile-hero{background-image:var(--profile-pattern,none);background-size:var(--profile-pattern-size,auto)}.overall-profile-card.cosmetic-stripe-cyan{--profile-pattern:repeating-linear-gradient(112deg,transparent 0 46px,color-mix(in srgb,var(--profile-accent) 15%,transparent) 47px 52px,transparent 53px 82px)}.overall-profile-card.cosmetic-stripe-apex{--profile-pattern:linear-gradient(118deg,transparent 0 68%,color-mix(in srgb,var(--profile-accent) 24%,transparent) 69% 72%,transparent 73%)}.overall-profile-card.cosmetic-stripe-chevron{--profile-pattern:repeating-linear-gradient(135deg,transparent 0 38px,color-mix(in srgb,var(--profile-accent) 14%,transparent) 39px 52px,transparent 53px 78px)}.overall-profile-card.cosmetic-stripe-grid{--profile-pattern:linear-gradient(color-mix(in srgb,var(--profile-accent) 10%,transparent) 1px,transparent 1px),linear-gradient(90deg,color-mix(in srgb,var(--profile-accent) 10%,transparent) 1px,transparent 1px);--profile-pattern-size:28px 28px}.overall-profile-card.cosmetic-stripe-circuit{--profile-pattern:radial-gradient(circle at 72% 30%,color-mix(in srgb,var(--profile-accent) 40%,transparent) 0 4px,transparent 5px),linear-gradient(155deg,transparent 72%,color-mix(in srgb,var(--profile-accent) 18%,transparent) 73% 74%,transparent 75%)}.overall-profile-card.cosmetic-stripe-scan{--profile-pattern:repeating-linear-gradient(0deg,transparent 0 7px,color-mix(in srgb,var(--profile-accent) 10%,transparent) 8px 9px)}.overall-profile-card.cosmetic-stripe-blocks{--profile-pattern:linear-gradient(90deg,transparent 0 52%,color-mix(in srgb,var(--profile-accent) 15%,transparent) 53% 65%,transparent 66% 70%,color-mix(in srgb,var(--profile-accent) 22%,transparent) 71% 84%,transparent 85%)}.profile-member-badge{display:inline-flex;padding:5px 8px;background:#6fe0b1;color:#102f36;font-size:12px;font-weight:700;letter-spacing:.8px}.profile-member-badge.compact{padding:2px 5px;font-size:9px;line-height:1;border-radius:3px;white-space:nowrap}
      .profile-customizer{border-left-color:var(--profile-accent,#7ee7ff)!important}.profile-customizer-body{grid-template-columns:minmax(310px,.8fr) minmax(0,1.55fr)!important;align-items:start}.profile-customizer-intro,.profile-customizer-actions{grid-column:1/-1}.profile-customizer-preview{position:sticky;top:8px;display:grid;gap:9px;min-width:0;padding:12px;background:#0c1838;border:1px solid color-mix(in srgb,var(--profile-accent,#7ee7ff) 48%,#263a70)}.profile-customizer-preview header{display:flex;justify-content:space-between;align-items:end;gap:8px}.profile-customizer-preview header b{color:var(--profile-accent,#7ee7ff);font-size:18px}.profile-customizer-preview header small{color:#b9cde8;font-size:12px}.profile-preview-stage{position:relative;display:grid;grid-template-columns:150px minmax(0,1fr);align-items:center;gap:10px;min-height:112px;padding:8px;background:#172653;overflow:hidden}.profile-preview-stage>.overall-car-model{width:150px!important;height:96px!important;margin:0!important}.profile-preview-stage>span{position:relative;z-index:3;display:flex;flex-direction:column}.profile-preview-stage>span b{font-size:20px}.profile-preview-stage>span small{color:var(--profile-accent,#9feaff);font-size:13px}.profile-row-showcase{padding:0!important;border:0!important;background:transparent!important}.profile-customizer-controls{display:grid;grid-template-columns:1fr 1fr;gap:10px;min-width:0}.profile-customizer-controls .profile-cosmetic-group{background:#132655}.profile-cosmetic-group>p{min-height:31px;margin:0 0 7px;color:#b9cde8;font-size:12px;line-height:1.25}.profile-cosmetic-preview{position:relative;display:block;width:48px;height:36px;overflow:hidden;background:linear-gradient(105deg,#344b82,#1a2b5c);border:1px solid rgba(255,255,255,.22)}.profile-cosmetic-preview i{position:absolute!important;inset:0!important;width:auto!important;height:auto!important;background:transparent}.profile-cosmetic-choice{grid-template-columns:54px minmax(0,1fr)!important;min-height:60px!important}.profile-cosmetic-choice.selected{border-color:var(--profile-accent,#fff)!important;box-shadow:inset 4px 0 0 var(--profile-accent,#7ee7ff)}.profile-cosmetic-choice[class*='cosmetic-accent-'] .profile-cosmetic-preview{background:var(--choice-accent,#7ee7ff)!important}.cosmetic-accent-white .profile-cosmetic-preview{--choice-accent:#f4f7ff}.cosmetic-accent-lime .profile-cosmetic-preview{--choice-accent:#9ff58d}.cosmetic-accent-gold .profile-cosmetic-preview{--choice-accent:#ffd56e}.cosmetic-accent-orange .profile-cosmetic-preview{--choice-accent:#ff9a55}.cosmetic-accent-coral .profile-cosmetic-preview{--choice-accent:#ff7f76}.cosmetic-accent-pink .profile-cosmetic-preview{--choice-accent:#ff82d9}.cosmetic-accent-violet .profile-cosmetic-preview{--choice-accent:#b9a0ff}.cosmetic-accent-ice .profile-cosmetic-preview{--choice-accent:#d8fbff}.profile-cosmetic-choice.cosmetic-finish-solid .profile-cosmetic-preview{background:#29437e!important}.profile-cosmetic-choice.cosmetic-finish-split .profile-cosmetic-preview{background:linear-gradient(110deg,#3b86a8 0 48%,#26305f 49%)!important}.profile-cosmetic-choice.cosmetic-finish-gloss .profile-cosmetic-preview{background:linear-gradient(165deg,rgba(255,255,255,.32) 0 20%,transparent 21%),linear-gradient(105deg,#3b86a8,#26305f)!important}.profile-cosmetic-choice.cosmetic-finish-carbon .profile-cosmetic-preview{background:repeating-linear-gradient(45deg,#344b82 0 6px,#1d2d5b 7px 12px)!important}.profile-cosmetic-choice.cosmetic-finish-horizon .profile-cosmetic-preview{background:linear-gradient(180deg,#3b86a8 0 45%,#ffe27a 46% 51%,#26305f 52%)!important}.profile-cosmetic-choice[class*='cosmetic-stage-tint-'] .profile-cosmetic-preview::after{content:'';position:absolute;inset:0;background:var(--choice-tint,transparent);mix-blend-mode:color;opacity:.65}.profile-cosmetic-choice.cosmetic-stage-tint-blue{--choice-tint:#247ee8}.profile-cosmetic-choice.cosmetic-stage-tint-teal{--choice-tint:#00a99b}.profile-cosmetic-choice.cosmetic-stage-tint-gold{--choice-tint:#e5a928}.profile-cosmetic-choice.cosmetic-stage-tint-red{--choice-tint:#d62d46}.profile-cosmetic-choice.cosmetic-stage-tint-pink{--choice-tint:#df4dab}.profile-cosmetic-choice.cosmetic-stage-tint-mono{--choice-tint:#8a92a1}.profile-cosmetic-choice[class*='cosmetic-emblem-'] .profile-cosmetic-preview::after{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;color:#eaf7ff;font-size:25px}.profile-cosmetic-choice.cosmetic-emblem-bolt .profile-cosmetic-preview::after{content:'⚡'}.profile-cosmetic-choice.cosmetic-emblem-star .profile-cosmetic-preview::after{content:'★'}.profile-cosmetic-choice.cosmetic-emblem-flag .profile-cosmetic-preview::after{content:'⚑'}.profile-cosmetic-choice.cosmetic-emblem-flame .profile-cosmetic-preview::after{content:'▲'}.profile-cosmetic-choice.cosmetic-emblem-crown .profile-cosmetic-preview::after{content:'♛'}.profile-favorite-track{grid-column:1/-1;display:grid;grid-template-columns:minmax(170px,.7fr) minmax(220px,1.3fr);align-items:center;gap:12px;padding:11px 13px;background:#1a2f64;border-left:4px solid var(--profile-accent,#7ee7ff);color:#fff}.profile-favorite-track>span{display:flex;flex-direction:column;gap:3px}.profile-favorite-track small{color:#b9cde8}.profile-favorite-track select{min-width:0;height:42px;padding:0 10px;background:#0f1d44;border:1px solid rgba(126,231,255,.5);color:#fff;font:15px ForcedSquare,Arial,sans-serif}
      .cosmetic-stage-tint-blue .overall-car-model::before,.cosmetic-stage-tint-teal .overall-car-model::before,.cosmetic-stage-tint-gold .overall-car-model::before,.cosmetic-stage-tint-red .overall-car-model::before,.cosmetic-stage-tint-pink .overall-car-model::before,.cosmetic-stage-tint-mono .overall-car-model::before{content:'';position:absolute;inset:0;z-index:1;pointer-events:none;mix-blend-mode:color;opacity:.5}.cosmetic-stage-tint-blue .overall-car-model::before{background:#247ee8}.cosmetic-stage-tint-teal .overall-car-model::before{background:#00a99b}.cosmetic-stage-tint-gold .overall-car-model::before{background:#e5a928}.cosmetic-stage-tint-red .overall-car-model::before{background:#d62d46}.cosmetic-stage-tint-pink .overall-car-model::before{background:#df4dab}.cosmetic-stage-tint-mono .overall-car-model::before{background:#8a92a1;mix-blend-mode:saturation;opacity:1}.overall-car-model{isolation:isolate}.overall-car-model>img{position:relative;z-index:2}.profile-customizer-preview .overall-car-model>img,.profile-car-column>.overall-car-model>img{z-index:2}
      .overall-category-control{position:relative;grid-template-columns:minmax(0,1fr)!important;gap:0!important}.overall-category-select select{padding-right:54px!important}.overall-category-info{position:absolute!important;right:5px;top:50%;z-index:4;width:34px!important;min-width:34px!important;height:32px;transform:translateY(-50%);border:0!important;border-left:1px solid rgba(126,231,255,.36)!important;background:#172b5d!important}.overall-category-info svg{width:20px;height:20px;fill:none;stroke:currentColor;stroke-width:2}.leaderboard-ui .verified-state.verified .sq-integrity-label{display:none}
      @media(max-width:1000px){.profile-customizer-body{grid-template-columns:1fr!important}.profile-customizer-preview{position:relative;top:auto}.profile-customizer-controls{grid-template-columns:1fr 1fr}}
      @media(max-width:700px){.profile-customizer-controls{grid-template-columns:1fr}.profile-customizer-preview header{align-items:flex-start;flex-direction:column}.profile-preview-stage{grid-template-columns:120px minmax(0,1fr)}.profile-preview-stage>.overall-car-model{width:120px!important;height:82px!important}.profile-favorite-track{grid-template-columns:1fr}.profile-customizer-actions{align-items:stretch;flex-direction:column}.profile-customizer-actions .button{width:100%}}
      ...
      /* Racer Studio v3 corrections. Accent owns every highlight, so it stays independent of the base colors. */
      .cosmetic-accent-cyan.cosmetic-accent-cyan{--profile-accent:#7ee7ff;--cosmetic-glow:#7ee7ff;--show-accent:#7ee7ff}
      .cosmetic-accent-white.cosmetic-accent-white{--profile-accent:#f4f7ff;--cosmetic-glow:#f4f7ff;--show-accent:#f4f7ff}
      .cosmetic-accent-lime.cosmetic-accent-lime{--profile-accent:#9ff58d;--cosmetic-glow:#9ff58d;--show-accent:#9ff58d}
      .cosmetic-accent-gold.cosmetic-accent-gold{--profile-accent:#ffd56e;--cosmetic-glow:#ffd56e;--show-accent:#ffd56e}
      .cosmetic-accent-orange.cosmetic-accent-orange{--profile-accent:#ff9a55;--cosmetic-glow:#ff9a55;--show-accent:#ff9a55}
      .cosmetic-accent-coral.cosmetic-accent-coral{--profile-accent:#ff7f76;--cosmetic-glow:#ff7f76;--show-accent:#ff7f76}
      .cosmetic-accent-pink.cosmetic-accent-pink{--profile-accent:#ff82d9;--cosmetic-glow:#ff82d9;--show-accent:#ff82d9}
      .cosmetic-accent-violet.cosmetic-accent-violet{--profile-accent:#b9a0ff;--cosmetic-glow:#b9a0ff;--show-accent:#b9a0ff}
      .cosmetic-accent-ice.cosmetic-accent-ice{--profile-accent:#d8fbff;--cosmetic-glow:#d8fbff;--show-accent:#d8fbff}
      .profile-row-showcase{--show-accent:var(--profile-accent,#7ee7ff)}
      /* Mini previews that used to fall back to the generic chip now show the layer they actually set. */
      .profile-cosmetic-choice.cosmetic-theme-classic .profile-cosmetic-preview{background:linear-gradient(105deg,#2d3b68,#1a2958)}
      .profile-cosmetic-choice.cosmetic-theme-cyan .profile-cosmetic-preview{background:linear-gradient(105deg,#086782,#263d78)}
      .profile-cosmetic-choice.cosmetic-stripe-standard .profile-cosmetic-preview{background:linear-gradient(105deg,#2f467e,#182958)}
      .profile-cosmetic-choice.cosmetic-stripe-cyan .profile-cosmetic-preview{background:repeating-linear-gradient(112deg,#1d2c5e 0 13px,rgba(126,231,255,.72) 14px 17px,#1d2c5e 18px 26px)}
      .profile-cosmetic-choice.cosmetic-stage-garage .profile-cosmetic-preview{background:linear-gradient(145deg,#1d2a55,#111c3c)}
      .profile-cosmetic-choice.cosmetic-stage-grid .profile-cosmetic-preview{background:repeating-linear-gradient(90deg,transparent 0 8px,rgba(255,255,255,.3) 9px 11px) bottom/100% 36% no-repeat,linear-gradient(165deg,#284786 0 60%,#f5db69 61% 64%,#172653 65%)}
      /* Racer title and displayed badge are text, so those groups drop the color chip entirely. */
      .profile-cosmetic-group.kind-title .profile-cosmetic-choice,.profile-cosmetic-group.kind-badge .profile-cosmetic-choice{grid-template-columns:minmax(0,1fr)!important}
      /* Long racer names and option labels wrap instead of being cut off in narrow columns. */
      .profile-cosmetic-choice>b,.profile-cosmetic-choice>small{min-width:0;overflow-wrap:break-word}
      .profile-preview-stage>span{min-width:0}
      .profile-preview-stage>span>b,.profile-preview-stage>span>small{min-width:0;overflow-wrap:break-word}
      .showcase-row>span,.showcase-row>span>b{min-width:0;overflow-wrap:break-word}
      /* Accent swatches must describe their own option, not the card's current accent. */
      .profile-cosmetic-choice.cosmetic-accent-cyan .profile-cosmetic-preview{--choice-accent:#7ee7ff}
      .profile-cosmetic-choice.cosmetic-accent-white .profile-cosmetic-preview{--choice-accent:#f4f7ff}
      .profile-cosmetic-choice.cosmetic-accent-lime .profile-cosmetic-preview{--choice-accent:#9ff58d}
      .profile-cosmetic-choice.cosmetic-accent-gold .profile-cosmetic-preview{--choice-accent:#ffd56e}
      .profile-cosmetic-choice.cosmetic-accent-orange .profile-cosmetic-preview{--choice-accent:#ff9a55}
      .profile-cosmetic-choice.cosmetic-accent-coral .profile-cosmetic-preview{--choice-accent:#ff7f76}
      .profile-cosmetic-choice.cosmetic-accent-pink .profile-cosmetic-preview{--choice-accent:#ff82d9}
      .profile-cosmetic-choice.cosmetic-accent-violet .profile-cosmetic-preview{--choice-accent:#b9a0ff}
      .profile-cosmetic-choice.cosmetic-accent-ice .profile-cosmetic-preview{--choice-accent:#d8fbff}
      /* One option per row on phones so names such as Monochrome are never split mid-word. */
      @media(max-width:460px){.profile-cosmetic-group>div{grid-template-columns:minmax(0,1fr)!important}}
      .overall-racer-title:empty,.profile-racer-title:empty,.showcase-badges:empty,.showcase-meta:empty{display:none}
      .showcase-row>span>b{display:flex;align-items:center;gap:6px;flex-wrap:wrap}
      .showcase-badges{display:inline-flex;align-items:center;gap:4px}
      .showcase-meta{display:flex;align-items:center;flex-wrap:wrap;gap:4px}
      .showcase-row .overall-racer-title{border-color:var(--show-accent);color:var(--show-accent)}
      .profile-preview-title:empty{display:none}
      /* Plain racer themes must flatten every v3 layer, not only the base colors and pattern. */
      .sq-plain-racer-themes [class*='cosmetic-emblem-'] .overall-name::before,.sq-plain-racer-themes .leaderboard-ui [class*='cosmetic-emblem-'] .name::before{display:none!important}
      .sq-plain-racer-themes [class*='cosmetic-stage-tint-'] .overall-car-model::before{display:none!important}
      .sq-plain-racer-themes .overall-entry[class*='cosmetic-stage-'] .overall-car-model,.sq-plain-racer-themes .leaderboard-ui button.main[class*='cosmetic-stage-'] .overall-car-model{background:#172653!important}
      .sq-plain-racer-themes .overall-entry[class*='cosmetic-stage-'] .overall-car-model::after,.sq-plain-racer-themes .leaderboard-ui button.main[class*='cosmetic-stage-'] .overall-car-model::after{display:none!important}
      .sq-plain-racer-themes .overall-entry.cosmetic-podium-override[class*='cosmetic-finish-']{background:#2d3b68!important}
      /* A kept gold first place still shows the racer's accent on its pattern, emblem and title, exactly like the real row. */
      .overall-profile-card:not(.cosmetic-podium-override) .showcase-first{--show-accent:var(--profile-accent,#7ee7ff);border-left-color:#ffe27a}
      .overall-profile-card:not(.cosmetic-podium-override) .showcase-first>em{color:#ffe27a}
      /* Studio controls use the same white keyboard ring as the rest of the Ranked panel. */
      .profile-customizer>summary:focus-visible,.profile-cosmetic-choice:focus-visible,.profile-favorite-track select:focus-visible,[data-cosmetic-podium]:focus-visible,[data-save-profile-cosmetics]:focus-visible{outline:2px solid #fff;outline-offset:2px}
    `;
  ...
  function ensurePanel(){
    ...
    panel.addEventListener('change',(event)=>{
      if(event.target?.matches?.('[data-cosmetic-favorite]')){
        ...
        const entryId=cleanUserId(card?.querySelector('[data-save-profile-cosmetics]')?.dataset.accountId||'');
        profileCosmeticDrafts.set(entryId,{...sanitizeProfileCosmetics(profileCosmeticDrafts.get(entryId)),favoriteTrackId:TRACK_CATALOG.has(String(event.target.value||''))?String(event.target.value):''});
        return;
      }
  ...
  function updateOverallPager(){
    ...
    const info=document.getElementById('overallCategoryInfo');
    if(info){const tip=LEADERBOARD_INFO[overallCategory]||'This ranking uses the current complete Ranked snapshot.';info.dataset.tip=tip;info.title=tip;info.setAttribute('aria-label',`About ${label}: ${tip}`);}
  ...
  function profileGuideMarkup(entry,self,isSelf,cachedFinishes,snapshotFinishes,selfFinishes){
    ...
    const improve=simulationFinishes.filter((finish)=>Number(finish.rank||0)>1&&Number(finish.fieldSize||0)>=2).map((finish)=>recommendationAction(finish,'improve',plannerEntry,simulationFinishes)).filter(rankedRoute);
    const store=trackSnapshotStore();
    const starts=[];
    for(const track of TRACK_CATALOG.values()){
      if(completed.has(track.id))continue;
      ...
      if(field<2)continue;
      const action=recommendationAction({trackId:track.id,rank:0,fieldSize:field,weight:cachedTrackWeight(track.id,snapshot),cachedAt:Number(snapshot?.serverUpdatedAt||snapshot?.fetchedAt||0)||0},'start',plannerEntry,simulationFinishes);
    ...
    const personal=[...improve,...starts].filter(rankedRoute).sort((a,b)=>b.value-a.value||b.weight-a.weight);
  ...
  function openRankedProfile(userId){
    ...
    applyProfileCosmetics(content.closest('.overall-profile-card'),cosmeticsForEntry(entry));
  ...
  async function saveRankedProfileCosmetics(accountId,status,button){
    ...
    try{
      ...
      batch.set(fire.collection('0.6.2_profiles_public').doc(safeId),{profileCosmetics:cosmetics,updatedAt:now},{merge:true});
      ...
      await batch.commit();
      if(status)status.textContent='Saved. Updating public leaderboards...';
      ...
      if(endpoint&&rankedEdgeAvailable()){
        try{
          ...
          clearRankedEdgeFailure();
          if(response.ok&&payload.accepted===true)status&&(status.textContent='Saved for everyone.');
          else if(payload.error==='cosmetic_locked')status&&(status.textContent='Saved on this device. The server has not unlocked that design yet.');
          else status&&(status.textContent='Saved. Public update queued.');
        }catch(error){
          markRankedEdgeUnavailable(String(error&&(error.message||error)));
          if(status)status.textContent='Saved. Public update queued.';
        }
      }else if(status)status.textContent='Saved. Public update queued.';
      return true;
    }catch(error){
      if(status)status.textContent=`Saved on this device. The public update could not be queued yet and will retry. (${String(error&&(error.message||error))})`;
      if(button)button.textContent='Try again';
      return false;
    }finally{
      if(button){button.disabled=false;setTimeout(()=>{if(button.isConnected&&button.textContent==='Saved')button.textContent='Save for everyone';},1400);}
    }
  }
  ...
  async function mirrorRaceResult(url, body){
    ...
    try {
      ...
      scheduleLocalPbCloudReconcile(accountId,permissionBlocked?600000:15000);
  ...
  function decorateNativeLeaderboardCosmetics(){
    ...
    const buttons=Array.from(document.querySelectorAll('.leaderboard-ui > .container > button.main'));
    if(!trackId||!buttons.length)return;
    ...
    for(const button of buttons){
      ...
      const cosmeticClasses=racerCosmeticClasses(racer).split(/\s+/).filter(Boolean);
