...
.map((track)=>[track.id, Object.freeze(track)]));
  ...
  function helpfulThresholdText(action){
    ...
    if(current===1)return 'Only a faster PB strengthens this result; slower runs change nothing';
    if(current>1)return `Beat your current #${current} PB; slower runs change nothing`;
  ...
  function guideTrackCard(action,maxValue,verb='Try'){
    ...
    const current=action.currentRank?`Existing PB #${action.currentRank} of ${action.fieldSize}`:'New track · no saved result';
    const rivalTarget=Number(action.rivalRank||0)>0?`Beat rival #${action.rivalRank} · `:'';
    const target=action.currentRank===1?'Keep #1 with a faster PB':`${rivalTarget}Aim #${action.targetRank} · ${helpfulThresholdText(action)}`;
  ...
  function profileGuideMarkup(entry,self,isSelf,cachedFinishes,snapshotFinishes,selfFinishes){
    ...
    const quickSummary=priority[0]?`<strong>${escapeHtml(trackInfo(priority[0].trackId).name)}</strong><small>${priority[0].currentRank?`Existing #${priority[0].currentRank} · faster PB toward #${priority[0].targetRank}`:`New track · aim #${priority[0].targetRank} · top ${priority[0].minimumHelpfulRank} helps`}</small>`:'<strong>Needs more track data</strong>';
